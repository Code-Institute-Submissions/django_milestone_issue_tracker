...
class TestUserAuthViews(TestCase):
    ...
    def test_user_exists(self):
        ...
        response = self.client.post('/accounts/register/', data, follow=True)
        self.assertContains(response, 'Email addresses is already registered.', 0, 200)
    ...
    def test_login_no_username(self):
        response = self.client.post('/accounts/login/', {'username_or_email': ''})
        self.assertContains(response, 'This field is required.', 2, 200)
        self.assertFormError(response, 'form', 'username_or_email', u'This field is required.')    
