...
def feature_detail(request, pk):
    ...
    feature.views += 1
    feature.save()
    comments = CommentFeature.objects.filter(feature=feature).order_by(
        '-created_date'
    )
    if request.method == 'POST':
        ...
        if comment_form.is_valid():
            ...
            comment = CommentFeature.objects.create(feature=feature,
...
def upvote_feature(request, pk):
    ...
    upvote = UpvoteFeature.objects.filter(feature=feature,
    ...
    if not upvote:
        ...
        messages.success(request, 'Upvote added to cart',
                         extra_tags="alert-success")
        return redirect(feature_detail, feature.pk)
    else:
        messages.error(request, 'You need to pay for your vote!',
