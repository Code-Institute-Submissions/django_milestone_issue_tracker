...
class TestBugViews(TestCase):
    ...
    def create_test_bug(self):
        bug = Bug(name='Test', description='Test description', author=self.user)
        bug.save()
        return bug
    ...
    def test_comment_bug(self):
        ...
        response = self.client.post('/bugs/{}/'.format(bug.id), data, follow=True)
        self.assertContains(response, 'You have successfully posted a comment', 1, 200)
        
    def test_upvote_bug(self):
        bug = self.create_test_bug(self)
        response = self.client.post('/bugs/{}/upvote/'.format(bug.id), follow=True)
